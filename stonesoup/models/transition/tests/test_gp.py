...
def test_gp(gp_model_params, sign):
    ...
    if dim == 1:
        ...
        model_obj = MarkovianGP(window_size=window_size,
        ...
        for i in range(dim):
            ...
            model_list.append(MarkovianGP(window_size=window_size,
                                          kernel_params=kernel_params))
    ...
    states = [
        State([[i + 1.0]], timestamp=start_time + datetime.timedelta(seconds=i*timediff))
    ...
    for i in range(0, dim):
        ...
        K = np.array([
            [se_kernel(train_times[i], train_times[j], l, var) for j in range(window_size)]
            for i in range(window_size)
        ])
    ...
    assert np.allclose(F, model_obj.jacobian(
        State(state_vec), time_interval=time_interval, track=track), rtol=1e-6)

    # Ensure ```model_obj.covar(time_interval)``` returns Q

    print(Q[0,0])
    print( model_obj.covar(
        timestamp=new_timestamp, time_interval=time_interval, track=track)[0,0])
    assert np.allclose(Q, model_obj.covar(
        timestamp=new_timestamp, time_interval=time_interval, track=track), rtol=1e-6)
    ...
    # print(inspect.getmro(type(model_obj)))
    ...
    # (with external noise)
    noise = model_obj.rvs(timestamp=new_timestamp, time_interval=time_interval, track=track)
